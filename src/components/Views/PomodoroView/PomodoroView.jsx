...
export default function PomodoroView() {
    const { minutes, seconds, isPaused, startTimer, pauseTimer } =
        useTimerContext()
    ...
    return (
        <motion.div
            ...
            onTap={() => setShowCloseButton(!showCloseButton)}
            ...
            style={{ width: showCloseButton ? "18rem" : "16rem" }}
        >
            <button
                onClick={() => {
                    setViews({ ...views, pomodoroView: false })
                    showHints.disableViewsHint === "disabled"
                        ? null
                        : setShowHints({ ...showHints, disableViewsHint: true })
                }}
            ...
            <p className="min-w-[10rem] text-start text-[3.6rem] leading-[3rem] pl-1 select-none">
                {minutes < 10 && minutes.toString().length == 1
                    ? "0" + minutes
                    : minutes}
