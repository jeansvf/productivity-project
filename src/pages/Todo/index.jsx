...
export default function Todo() {
    ...
    const deleteColumn = async (columnIndex) => {
        ...
        newColumnsOrder.map((column, index) =>
            column == selectedColumnId ? newColumnsOrder.splice(index, 1) : null
        )
        ...
        let snapshot = await getDocs(
            query(
                ...
                where("columnOrigin", "==", columns[columnIndex].id)
            )
        )
    ...
    const handleOnDragEnd = (result) => {
        ...
        if (result.type == "card") {
            ...
            columns.map((column, columnIndex) =>
                column.droppableColumnId == result.source.droppableId
                    ? (cardColumnSourceId = columnIndex)
                    : null
            )

            let cardColumnDestinationId
            columns.map((column, columnIndex) =>
                column.droppableColumnId == result.destination.droppableId
                    ? (cardColumnDestinationId = columnIndex)
                    : null
            )

            let selectedCard =
                columns[cardColumnSourceId].cards[result.source.index]
            ...
            newColumns[cardColumnDestinationId].cards.splice(
            ...
            updateDoc(
                doc(
                    db,
                    `users/${user.uid}/columns/${newColumns[cardColumnSourceId].id}`
                ),
                {
            ...
            updateDoc(
                doc(
                    db,
                    `users/${user.uid}/columns/${newColumns[cardColumnDestinationId].id}`
                ),
                {
    ...
    return (
        <main className="relative flex -mb-[12rem] pl-4 pt-20 w-screen h-screen overflow-x-auto bg-[#393939] text-white z-10 overflow-y-hidden">
            {isDataRetrieved ? (
                <>
                    <div>
