...
export default function Settings({setPomodoroConfigOpened}) {
    ...
    const addAlarmMinutesToLocalStorage = (timerType, newTime) => {
        ...
        let newAlarmSettings = JSON.parse(
            localStorage.getItem("alarm_settings")
        )
            ? JSON.parse(localStorage.getItem("alarm_settings"))
            : {}
    ...
    return (
        ...
        >
            <div className="flex justify-between w-[24rem] text-xl max-sm:w-[20rem] max-sm:text-base">
                <button
                    onClick={() => setSelectedTimer("pomodoro")}
                    className={`py-1 px-2 rounded-[0.3rem] text-black ${
                ...
                <button
                    onClick={() => setSelectedTimer("long_break")}
                    className={`py-1 px-2 rounded-[0.3rem] text-black ${
                        selectedTimer == "long_break"
                            ? "bg-[#fff082]"
                            : "text-white"
                    } `}
                    type="button"
                ...
                <button
                    onClick={() => setSelectedTimer("short_break")}
                    className={`py-1 px-2 rounded-[0.3rem] text-black ${
                        selectedTimer == "short_break"
                            ? "bg-[#84FF82]"
                            : "text-white"
                    }`}
                    type="button"
            ...
            <AlarmSoundSettings
                setPomodoroConfigOpened={setPomodoroConfigOpened}
            />
